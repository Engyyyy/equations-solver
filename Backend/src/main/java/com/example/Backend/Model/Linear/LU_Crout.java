...
public class LU_Crout extends Gauss {
    //do matrix of coeff
    ...
    public static double[][] gaussMat(double[][] mat, int n ,int prec ){
        ...
        for(int k=0; k<n; k++) {
            for (i=k; i<n-1; i++) {
                l= precision(mat[k][i+1]/ mat[k][k],prec);
                l=precision(l,prec);
                L[m]=l;
                count++;
                for( j=0; j<n; j++) {
                    mat[j][i+1]= precision((-1*L[m]*mat[j][k])+mat[j][i+1],prec);
                    mat[j][i+1]= precision(mat[j][i+1],prec);
                }
                m++;
            }
        }
    ...
    //generate L
    public static double[][] Do_U( int n, double []L) {
        long startTime = System.nanoTime();
    ...
    //generate U
    public static double[][] Do_L(double[][] mat, int n) {
        long startTime = System.nanoTime();
    ...
    public static double[][]forwardSub (double[][]mat ,double[][] B,  int n, int prec) {
        ...
        for(int r=0;r<n;r++) {
            for(int c=0 ; c<n;c++) {
                if(c==r) {
                    Y[r][0]=precision(NewAug[r][n]/NewAug[r][c],prec);
                }
            }
        }
    ...
    public static double[][] backwordSub (double[][]mat ,double[][] B,  int n ,int prec) {
        ...
        for(int r=0;r<n;r++) {
            X[r][0]=precision(NewAug[r][n],prec);
        }
